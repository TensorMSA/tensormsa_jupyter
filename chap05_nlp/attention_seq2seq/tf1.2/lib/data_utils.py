...
def get_batch(chat_token_ids, config):
	...
	for i in range(batch_size_t):
		encoder_len = len(chat_token_ids[rnd_idx[i]][0])
		decoder_len = len(chat_token_ids[rnd_idx[i]][1])
		if(encoder_len == 0 or decoder_len == 0) :
			batch_size_t = batch_size_t- 1
			continue
		encoder_inputs.append(chat_token_ids[rnd_idx[i]][0])
		decoder_inputs.append(chat_token_ids[rnd_idx[i]][1])
		encoder_inputs_length.append(encoder_len)
		decoder_inputs_length.append(decoder_len)

	if (len(encoder_inputs_length) > 0 ) :
