...
def main(mode):
    if('train' == mode):
        ...
        train_docs = [(tokenize_mecab(row), 1) for row in train_data]
        print(u'[OK]')
        ...
        sentence = input(u'Input sentance : ')
        sentence_avg_vector = avg_feature_vector(tokenize_mecab(sentence), model=ref_model, num_features=300)
        ...
        train_file = open("train_findpeople.txt")
        try:
            for line in train_file:
                ref_sentence_avg_vector = avg_feature_vector(tokenize_mecab(line), model=ref_model, num_features=300)
        ...
        ###########################
        # NER Testing...
        entity_file = open("entity_findpeople.txt")
        try:
